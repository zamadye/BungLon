...
| **Web: macet di "MEMUAT …" / visual game tidak muncul** | Setelah 9 dtk splash **selalu** membuka panel bantuan (`#splashHelp`) yang menyebut sebabnya; kotak `#splashDiag` berisi diagnosis lengkap (status HTTP tiap file, tipe MIME, ukuran, error + barisnya) dengan tombol **SALIN DIAGNOSIS**. Ikuti pesan di sana: server mati / dibuka sebagai `file://` → jalankan `node web/net-server.js` lalu buka `http://localhost:8790/`; **"game … HTTP 200 tapi isinya HTML"** = cache Service Worker lama yang rusak → tekan **↻ MUAT ULANG BERSIH** (melepas SW + mengosongkan cache; bila kegagalan yang sama terulang 2×, game melakukannya sendiri setelah hitung mundur 3 detik); **"SyntaxError"** = browser terlalu lama — build web ini butuh **Chrome/Edge 80+, Firefox 72+, Safari 13.1+**. Manual: `window.BungBoot` = `{state, missing, slow, done, total, ms}`, `window.BungBootInline.diag()` dan `.missing()`. |
| **Web: `?nosw=1` terasa tidak menolong** | Service Worker yang *sudah aktif* memang tetap mencegat semua request walau pendaftaran dilewati. Sejak versi ini `?nosw=1` juga **melepas registrations + mengosongkan cache lalu reload sekali** (penanda `sessionStorage['hs_nosw_done']`), dan `sw.js` `v2.3.1-safe` menolak menyimpan respons HTML untuk file `.js/.css/.json` sehingga cache beracun tidak terbentuk lagi. |
...
> **Aturan sintaks browser** (`web/*.js` yang dimuat `<script>`): **ES2019 saja** — tanpa `?.`, `??`,
> `||=`/`&&=`, `#field` privat, `static {}`, `.at(-1)`, `Object.hasOwn`, `.replaceAll`, `Promise.any`.
> Satu token ES2020 di salah satu file membuat browser lama membuang **seluruh** file itu
> (`SyntaxError`) sehingga game tidak pernah mulai — persis gejala "stuck di loading". Dikunci oleh
> `tools/web_boot_test.js` grup `[G]`, jadi tidak bisa lolos diam-diam.
...
| `web/manifest.webmanifest` + `web/sw.js` | PWA: Add to Home Screen, offline (app-shell precache), shortcut Solo/Room. `?nosw=1` = lepas SW aktif + kosongkan cache (+`sw.js` menolak menyimpan respons HTML untuk `.js/.css/.json`, jadi cache tidak bisa "beracun") |
| `web/index.html` (blok `<script>` pertama) | **watchdog boot lapis pertama** (`window.BungBootInline`): ES5-safe, tidak bergantung file mana pun — mendeteksi `<script src>` yang globalnya tidak pernah muncul, memeriksa status HTTP/tipe/ukuran tiap file, menulisnya ke `#splashDiag` (+ tombol SALIN DIAGNOSIS), dan melepas SW + cache sendiri bila kegagalan yang sama terulang |
